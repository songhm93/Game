...
enum class UAV_REGISTER : uint8 //컴퓨트 셰이더에 넘겨줄 레지스터
{
...
struct Vertex //IV단계에 꽂아줄 버텍스 정보. Shader.cpp D3D12_INPUT_ELEMENT_DESC과 셰이더파일에 struct VS_IN을 참고
{
	Vertex() {}
	Vertex(Vec3 pos, Vec2 uv, Vec3 normal, Vec3 tangent) : pos(pos), uv(uv), normal(normal), tangent(tangent)
	{
	}
	Vec3 pos;
	Vec2 uv;
...
extern unique_ptr<Engine> GEngine; //전역 extern 변수
