...
void FBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	uint32 arrIdx[3];
	uint32 vertexCounter = 0; // 정점의 갯수

	const int32 triCount = mesh->GetPolygonCount(); // 메쉬의 삼각형 갯수를 가져온다.
	for (int32 i = 0; i < triCount; i++) //삼각형의 갯수
	{
		for (int32 j = 0; j < 3; j++) // 삼각형은 3개의 정점으로 구성
		{
...
void FBXLoader::GetTangent(FbxMesh* mesh, FbxMeshInfo* meshInfo, int32 idx, int32 vertexCounter)
{
	...
	{
		// TEMP : 원래는 이런저런 알고리즘으로 Tangent 만들어줘야함
