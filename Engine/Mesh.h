...
public:
	...
	void Render(shared_ptr<InstancingBuffer>& buffer, uint32 idx = 0);

	static shared_ptr<Mesh> CreateFromFBX(const struct FbxMeshInfo* meshInfo, FBXLoader& loader);
...
public:
	...
	shared_ptr<StructuredBuffer> GetBoneFrameDataBuffer(int32 index = 0) { return _frameBuffer[index]; }
