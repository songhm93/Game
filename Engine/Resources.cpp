...
shared_ptr<Mesh> Resources::LoadCubeMesh()
{
	...
	float w2 = 0.5f; // width/2 = 0.5 전체 width가 1
	float h2 = 0.5f;
	float d2 = 0.5f;
...
shared_ptr<Mesh> Resources::LoadSphereMesh()
{
	...
	uint32 sliceCount = 20; // 세로 분할 얘네를 늘리면 원의 형태가 더 매끄러워지지만 부하는 많아짐
...
shared_ptr<Mesh> Resources::LoadTerrainMesh(int32 sizeX, int32 sizeZ)
{
	...
	for (int32 z = 0; z < sizeZ + 1; z++) //이중for문을 돌면서 정점을 만듬
	{
...
void Resources::CreateDefaultShader()
{
	// Skybox
	{
		...
		Add<Shader>(L"Skybox", shader); //Map에 Add해주는 부분. 키를 string으로, 밸류를 공유포인터로.
	}
	...
	// Tessellation
	{
		...
		{
			SHADER_TYPE::FORWARD,
			RASTERIZER_TYPE::WIREFRAME, //와이어 프레임.
			DEPTH_STENCIL_TYPE::LESS,
			BLEND_TYPE::DEFAULT,
			D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST //컨트롤포인트 패치리스트
		};
...
void Resources::CreateDefaultMaterial()
{
	...
	// DirLight
	{
		...
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		material->SetTexture(0, GET_SINGLE(Resources)->Get<Texture>(L"PositionTarget")); //순서 유의 0번 1번
	...
	// GameObject
	{
		...
		shared_ptr<Texture> texture = GET_SINGLE(Resources)->Load<Texture>(L"Leather", L"..\\Resources\\Texture\\Leather.jpg"); //albedo
		shared_ptr<Texture> texture2 = GET_SINGLE(Resources)->Load<Texture>(L"Leather_Normal", L"..\\Resources\\Texture\\Leather_Normal.jpg"); //normal
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		material->SetTexture(0, texture); //albedo를 넘겨준 것
		material->SetTexture(1, texture2);//normal을 넘겨준 것
	...
	// Terrain
	{
		...
		shared_ptr<Texture> texture = GET_SINGLE(Resources)->Load<Texture>(L"Terrain", L"..\\Resources\\Terrain\\Terrain.png"); //albedo
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
