...
void StructuredBuffer::PushGraphicsData(SRV_REGISTER reg) //t9
{
	GEngine->GetGraphicsDescHeap()->SetSRV(_srvHeapBegin, reg);
}

void StructuredBuffer::PushComputeSRVData(SRV_REGISTER reg) //이건 아직 안쓰는중
{
	GEngine->GetComputeDescHeap()->SetSRV(_srvHeapBegin, reg);
}

void StructuredBuffer::PushComputeUAVData(UAV_REGISTER reg) //u0, u1
{
