...
void Transform::FinalUpdate()//행렬 변환이 일어나는 곳.
{
	Matrix matScale = Matrix::CreateScale(_localScale); //S
	Matrix matRotation = Matrix::CreateRotationX(_localRotation.x); //R
	matRotation *= Matrix::CreateRotationY(_localRotation.y);
	matRotation *= Matrix::CreateRotationZ(_localRotation.z);
	Matrix matTranslation = Matrix::CreateTranslation(_localPosition); //T

	_matLocal = matScale * matRotation * matTranslation;
	_matWorld = _matLocal; //부모가 없는 경우는 SRT를 곱해서 끝난게 월드 행렬

	shared_ptr<Transform> parent = GetParent().lock(); //부모가 있는 경우
	if (parent != nullptr)
	{
		_matWorld *= parent->GetLocalToWorldMatrix(); //부모의 월드 행렬을 자기자신의 월드 행렬과 곱함
	}
}
...
void Transform::PushData()
{
	...
	CONST_BUFFER(CONSTANT_BUFFER_TYPE::TRANSFORM)->PushGraphicsData(&transformParams, sizeof(transformParams)); //버퍼에 담는다.
}
