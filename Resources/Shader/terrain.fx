...
PatchTess ConstantHS(InputPatch<VS_OUT, 3> input, int patchID : SV_PrimitiveID)
{
    ...
    float minDistance = g_vec2_1.x; //테셀레이션이 적용되는 최소거리와 최대거리
...
[outputcontrolpoints(3)] // 하나의 입력 패치에 대해, HS가 출력할 제어점 갯수
...
DS_OUT DS_Main(const OutputPatch<HS_OUT, 3> input, float3 location : SV_DomainLocation, PatchTess patch)
{
    ...
    float height = g_tex_2.SampleLevel(g_sam_0, fullUV, 0).x; //여기선 g_tex_2가 높이맵. x값을 추출해서 height 값으로...?
